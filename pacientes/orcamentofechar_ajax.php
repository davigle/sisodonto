...
<?
	$estados = array('À vista', 'Cheque pré-datado', 'Promissória', 'Desconto em folha', 'Cartão');
...
?>       
            ...
            <tr>
              <td width="5%">&nbsp;</td>
              <td width="28%"><div align="left">Nº DE PARCELAS:&nbsp;&nbsp;
        ...
        <form id="form3" name="form3" method="POST" action="pacientes/orcamentofechar_ajax.php?codigo=<?=$_GET[codigo]?>&acao=editar&subacao=editar&codigo_orc=<?=$codigo_orc?>" onsubmit="formSender(this, 'conteudo'); return false;"> &nbsp;<br />
          <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td bgcolor="#0099CC"><div align="center" class="style4">SITUAÇÃO</div></td>
...
?>
          <tr>
    		  <td><div align="center"><b>Parcela <?=$i?></b> <?=(($row1['codigo'] != '')?'(Boleto nº '.$row1['codigo'].')':'')?></div></td>
    		  <td><div align="center">
    		  ...
    		  <td><div align="center"><?=(($row['baixa'] == 'Não')?(($row1['pago'] == 'Sim')?'PAGO':'<a href="javascript:Ajax(\'pagamentos/parcelas\', \'conteudo\', \'codigo='.completa_zeros($row1['codigo'], ZEROS).'\')">ABERTO').((($row1['datavencimento'] < date('Y-m-d')) && ($row1['pago'] != 'Sim'))?' (VENCIDO)</a>':'</a>').(($row1['pago'] == 'Sim')?' ('.converte_data($row1['datapgto'], 2).')':''):(($row1['pago'] == 'Sim')?'PAGO ('.converte_data($row1['datapgto'], 2).')':'CANCELADO'))?></div></td>
    		  <td><div align="center">
...
?>
			<tr>
  			...
  			<tr>
              <td align="left"><input <?=$disable?> type="checkbox" <?=(($row['confirmado'] == 'Sim')?'checked':'')?> name="confirmed" id="confirmed" value="Sim"><label for="confirmed"> Orçamento confirmado</label></td>
...
<table width="100%" align="center">
  <tr>
    <td width="33%" align="center">
      <a href="relatorios/orcamento.php?codigo=<?=$codigo_orc?>" target="_blank">Imprimir Orçamento</a>
    </td>
<?
    if($disable == 'disabled') {
        if($row['baixa'] == 'Não') {
...
?>
    <td width="33%" align="center">
      <a href="javascript:;" onclick="if(confirm('Deseja realmente cancelar o restante das parcelas deste orçamento?\n\nAs parcelas já pagas não serão afetadas.')) { javascript:Ajax('pacientes/orcamentofechar', 'conteudo', 'codigo=<?=$_GET[codigo]?>&indice_orc=<?=($i+1)?>&acao=editar&subacao=editar&codigo_orc=<?=$row[codigo]?>&confirm_baixa=baixa') }">Dar Baixar no Orçamento</a>
    </td>
<?
            }
?>
