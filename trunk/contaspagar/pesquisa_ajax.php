...
<?
	...
	switch ($_GET[peri]) {
		case 'dia': {
			$lista = $conta->ListConta("SELECT * FROM `contaspagar` WHERE `datavencimento` = '$data' ORDER BY `datavencimento` ASC, `codigo` ASC");
		} break;
		case 'mes': {
			$lista = $conta->ListConta("SELECT * FROM `contaspagar` WHERE LEFT(`datavencimento`, 7) = '$data' ORDER BY `datavencimento` ASC, `codigo` ASC");
		} break;
		case 'mesatual': {
			$lista = $conta->ListConta("SELECT * FROM `contaspagar` WHERE LEFT(`datavencimento`, 7) = '".date(Y.'-'.m)."' ORDER BY `datavencimento` ASC, `codigo` ASC");
		} break;
	}
	$par = "F0F0F0";
	$impar = "F8F8F8";
