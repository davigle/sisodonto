<?
    ...
    */
	...
	if($_GET[confirm_del] == "delete')") {
        $sql = "UPDATE `dentistas` SET `foto` = '' WHERE `cpf` = '".$_GET['cpf']."'";
	...
	if(isset($_POST[send])) {
		if($_FILES['foto']['name'] != "") {
            ...
            $sql = "UPDATE `dentistas` SET `foto` = '".$img_data."' WHERE `cpf` = '".$_GET['cpf']."'";
            unlink('teste.jpg');
            mysql_query($sql) or die(mysql_error());
		}
	}
...
<?
    $sql = "SELECT `foto` FROM `dentistas` WHERE `cpf` = '".$_GET['cpf']."'";
