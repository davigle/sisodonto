...
<?
		...
		switch ($_GET[peri]) {
			case 'dia': {
				$lista = $conta->ListConta("SELECT * FROM `contasreceber_dent` WHERE `datavencimento` = '$data' AND `codigo_dentista` = '$codigo_dentista' ORDER BY `datavencimento` ASC, `codigo` ASC");
			} break;
			case 'mes': {
				$lista = $conta->ListConta("SELECT * FROM `contasreceber_dent` WHERE LEFT(`datavencimento`, 7) = '$data' AND `codigo_dentista` = '$codigo_dentista' ORDER BY `datavencimento` ASC, `codigo` ASC");
			} break;
			case 'mesatual': {
				$lista = $conta->ListConta("SELECT * FROM `contasreceber_dent` WHERE LEFT(`datavencimento`, 7) = '".date(Y.'-'.m)."' AND `codigo_dentista` = '$codigo_dentista' ORDER BY `datavencimento` ASC, `codigo` ASC");
			} break;
		}
		$par = "F0F0F0";
		$impar = "F8F8F8";
...
?>
    <tr bgcolor="#<?=$odev?>" onmouseout="style.background='#<?=$odev?>'" onmouseover="style.background='#DDE1E6'">
      ...
      <td width="5%" align="center"><a href="javascript:Ajax('contasreceber_dent/extrato', 'conteudo', 'codigo=<?=$conta->RetornaDados('codigo')?>&codigo_dentista=<?=$_GET[codigo_dentista]?>&senha_dentista=<?=$_GET[senha_dentista]?>" onclick="return confirmLink(this)"><img src="imagens/icones/excluir.gif" alt="Excluir" width="19" height="19" border="0"></a></td>
    </tr>
<?
	}
	if($odev == $impar) {
